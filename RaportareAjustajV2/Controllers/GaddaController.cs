...
namespace RaportareAjustajV2.Controllers
{
    public class GaddaController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            List<GaddaModel> listaDeAfisat = await _context.GaddaModels.OrderByDescending(t => t.DataIntroducere).ToListAsync();
        ...
        public async Task<IActionResult> ExportToExcelAsync(string dataFrom, string dataTo)
        {
            ...
            List<GaddaModel> listaSql = await _context.GaddaModels.ToListAsync();
            // Extrage datele cuprinse intre limitele date de operator
            IEnumerable<GaddaModel> listaDeAfisat = listaSql.Where(model => CalculeAuxiliar.IsDateBetween(model.DataIntroducere, dataFrom, dataTo));
        ...
        public async Task<IActionResult> Create([Bind("GaddaModelId,UserName,DataIntroducere,Cuptor,TratamentTermic,Diametru,Calitate,Sarja,NumarBare,LungimeBare,Masa,DataIncarcare,OraIncarcare,DataDescarcare,OraDescarcare,ConsumGaz,ConsumElectricitate")] GaddaModel gaddaModel)
        {
            ...
            @ViewBag.UserName = gaddaModel.UserName;
            return View(gaddaModel);
        }
