...
namespace RaportareAjustajV2.Controllers
{
    public class RullatriceProjectManController : Controller
    {
        ...
        public async Task<IActionResult> ExportToExcelAsync(string dataFrom, string dataTo)
        {
            ...
            List<RullatriceProjectManModel> listaSql = await _context.RullatriceProjectManModels.ToListAsync();
            // Extrage datele cuprinse intre limitele date de operator
            IEnumerable<RullatriceProjectManModel> listaDeAfisat = listaSql.Where(model => CalculeAuxiliar.IsDateBetween(model.DataIntroducere, dataFrom, dataTo));
        ...
        public async Task<IActionResult> Create([Bind("RullatriceProjectManModelId,UserName,DataIntroducere,DataControl,Diametru,Calitate,Sarja,NrBare,Motiv,Lungime,Masa")] RullatriceProjectManModel rullatriceProjectManModel)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("RullatriceProjectManModelId,UserName,DataIntroducere,DataControl,Diametru,Calitate,Sarja,NrBare,Motiv,Lungime,Masa")] RullatriceProjectManModel rullatriceProjectManModel)
        {
